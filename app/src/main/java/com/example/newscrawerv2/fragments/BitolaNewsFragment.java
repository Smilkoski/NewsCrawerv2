...
public class BitolaNewsFragment extends Fragment implements CustomListAdapter.OnArticleListener {
    ...
    private static Article scanBitolaNews(Document doc) {
        try {
            ...
            String vreme = doc.getElementsByClass("post-byline")
                    .first()
                    .ownText();
            String[] parts = vreme.split(" ");
            vreme = parts[1] + LocalDate.now().getMonth().getValue() + "," + parts[2];
            Date date = new SimpleDateFormat("dd,mm,yyyy").parse(vreme);

            String imgSrc = doc.getElementsByClass("featured-image")
                    .first()
                    .select("img")
                    .first()
                    .attr("src");
    ...
    public void generateAllArticles() {
        ...
        List<String> urls = Arrays.asList(
                "https://bitolanews.mk/category/bitola/",
                "https://bitolanews.mk/category/bitola/page/2/");
    ...
    private static class ScanArticleTask extends AsyncTask<String, Void, Article> {
        ...
        protected Article doInBackground(String... urls) {
            try {
                ...
                return scanBitolaNews(document);
